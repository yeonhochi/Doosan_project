...
def main(args=None):
    ...
    # ✅ 컵을 뽑는 위치
    pick_position = np.array([328.351, 145.223, 232.923, 24.416, -179.317, 9.374])
...
# ✅ 컵 좌표 리스트 생성
    ...
    # ✅ 새로운 순서로 재배열
    ordered_indices1 = [1, 3, 5, 8, 10, 12]
    ordered_indices2 = [2,4,9,11]
    ordered_indices3 = [3,10]  # ✅ 재배치된 순서
    ...
    for index, pose in enumerate(reordered_cup_positions1):
        ...
        lift_pose2 = trans(posx(lift_pose.tolist()), [0, 0, -150, 0, 0, 0], DR_BASE, DR_BASE)
...
################## 2 층 #####################
    ...
    for index, pose in enumerate(reordered_cup_positions2):
        ...
        lift_pose2 = trans(posx(lift_pose.tolist()), [0, 0, -55, 0, 0, 0], DR_BASE, DR_BASE)
        movel(lift_pose2, vel=VELOCITY, acc=ACC, ref=DR_BASE)
